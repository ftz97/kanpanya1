    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="w-full h-[600px] rounded-lg shadow bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center">
        <div className="text-center text-gray-500">
          ...
          <div className="text-lg font-semibold mb-2">Carte Interactive</div>
          <div className="text-sm">
            {zones.length > 0 ? (
              <>
                <div>📍 {zones.length} adresse{zones.length > 1 ? 's' : ''} ajoutée{zones.length > 1 ? 's' : ''}</div>
                <div className="mt-2 text-xs">
                  Utilisez la barre de recherche ci-dessus pour ajouter des adresses
                </div>
              </>
            ) : (
              <div>Utilisez la barre de recherche ci-dessus pour ajouter des adresses</div>
            )}
          </div>
        </div>
      </div>
